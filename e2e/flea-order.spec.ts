 ...
 *
 * 화면과 서버가 같은 값을 봐야 한다. 랜딩이 현장가를 적어 두고 주문 화면이
 * 정가로 열리면, 사람은 동의한 적 없는 금액을 청구받는다.
 *
 * 근거: [피그마 0uW99BqaTJKUVlowzQswli / 8-2. Rending Page]
 *       5472:1482 "방문수령 외 택배 시 배송비 3,000원 별도"
 *       5472:1755 "선착순 70명 예약하고, 과기대에서 수령하기"
 *
 * 값은 9/9 에 11,900 → 13,900 → 14,900, 9/11 에 18,900 으로 올렸다.
 * 피그마는 아직 11,900 이다.
 */
...
test.describe("플리마켓 주문", () => {
  ...
  test("들어온 경로를 서버에 알린다", async ({ page }) => {
    // 값이 갈리는 판정은 서버가 한다. 화면이 혼자 현장가를 그려 놓고
  ...
  test("현장 수령이 기본이고, 그때 값은 18,900원이다", async ({ page }) => {
    ...
    await expect(consent(page)).toContainText("제작비 18,900원");
    await expect(consent(page)).toContainText("방문수령(배송비 없음)");
    await expect(consent(page)).toContainText("18,900원을 결제하는 데");
  });
  ...
  test("택배로 바꾸면 주소를 묻고 배송비가 붙는다", async ({ page }) => {
    ...
    // 제작비 18,900 + 배송비 3,000 = 21,900
    await expect(consent(page)).toContainText("배송비 3,000원");
    await expect(consent(page)).toContainText("21,900원을 결제하는 데");
  });
  ...
  test("접수되면 어디로 넣을지가 화면에 뜬다", async ({ page }) => {
    ...
    await expect(page.locator(".gsf-payment-notice")).toContainText("18,900원");
