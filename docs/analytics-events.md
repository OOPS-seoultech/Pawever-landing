...
**① STEP 14는 건너뛸 수 있습니다.**
사연을 쓰지 않은 사람은 13 → 15로 바로 갑니다. 퍼널에서 14가 비어 보이는 것이 정상입니다.
...
| 신청 완료 | `application_complete` | STEP 15까지 끝냄 = 굿즈 신청 (서버 저장 완료 시점) |

최종 완료율을 어느 쪽으로 보실지 정해주시면 그 기준에 맞추겠습니다.
...
| `survey_complete` | STEP 13 완료 | `active_ms`, `goods_reserved` |
| `story_start` / `story_skip` | 사연 작성/건너뛰기 | `reason` |
| `production_form_view` | 굿즈 정보 화면 | `goods_type` |
| `application_complete` | **신청 완료** | `goods_type`, `photo_count`, `furthest_step` |
...
`탐색 → 자유 형식`에서 행에 `세션 소스/매체`, 값에 `application_complete` 이벤트 수.
